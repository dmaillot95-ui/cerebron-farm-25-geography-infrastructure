...
def payload_for(spec,prompt):
    ...
    for p in spec.get('parameters',[]):
        ...
        elif lname in {'system','system_prompt'}: payload[name]='Evidence-grounded infrastructure analysis. CLAIM<=EVIDENCE.'
...
role=os.environ['ROLE']; model=os.environ.get('MODEL','huggingface-projects/llama-3.2-3B-Instruct'); mission=pathlib.Path('MISSION.md').read_text()
prompt=f'''You are role {role} in CEREBRON Farm 25 Geography Infrastructure.\n{mission}\nProduce a concise infrastructure assessment with: spatial scope, demand/service assumptions, assets and networks, capacity and bottlenecks, interdependencies, hazards/failure modes, lifecycle/maintenance, cost and phasing boundaries, validation path, unknowns, and a final claim ledger. Never confuse map/model/simulation with field evidence.'''
ok,text,meta=invoke(model,prompt)
out={'role':role,'model':model,'inference_success':bool(ok),'status':'UNREVIEWED_EXTERNAL_AGENT_OUTPUT' if ok else 'EXTERNAL_INFERENCE_FAILED','text':text if ok else None,'error':None if ok else meta.get('error'),'meta':meta}
pathlib.Path('results').mkdir(exist_ok=True); pathlib.Path(f'results/{role}.json').write_text(json.dumps(out,ensure_ascii=False,indent=2)); print(json.dumps({'role':role,'inference_success':bool(ok),'model':model}))
